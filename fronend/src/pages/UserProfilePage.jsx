...
export default function UserProfilePage() {
  ...
  if (error) {
    if (error?.response.status) return <NotFoundPage />;
  }

  if (error) return "error";

  if (isLoading || isUserLoading) return <CircularProgress />;
  ...
  return (
    <>
      ...
      {isCentered && (isLoading || isUserLoading) ? <CircularProgress /> : null}
        ...
        >
          ...
          >
            ...
            {isCurrentUserProfile && (
              <Button component={Link} to="/dashboard/settings/profile/" variant="outlined" sx={{ px: 4, py: 1, borderRadius: "20px" }}>
                تنظیمات حساب کاربری
              </Button>
            )}
          </Box>
